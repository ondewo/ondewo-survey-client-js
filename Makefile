...
IMAGE_UTILS_NAME=ondewo-survey-client-utils-js:${ONDEWO_SURVEY_VERSION}
PRETTIER_WRITE?=

CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| perl -ne 'print if /Release ONDEWO Survey Js Client ${ONDEWO_SURVEY_VERSION}/../^\*{5}/'`

GH_REPO="https://github.com/ondewo/ondewo-survey-client-js"
...
check_build: ## Checks if all built proto-code is there
	...
	do \
		cat api/ondewo_survey_api.js | grep -q $${file};\
		if test $$? != 0; then  echo "No Proto-Code for $${file} in api/ondewo_survey_api.js" & exit 1;fi \
	done
	@rm -rf build_check.txt
...
# by hand, `git commit` exits 1 on a clean tree and would take the whole target -- publish,
# branch, tag, GitHub release -- down with it. spc still refuses an existing branch or tag.
	-git commit --no-verify -m "Preparing for Release ${ONDEWO_SURVEY_VERSION}"
	git push
...
build: check_out_correct_submodule_versions build_compiler update_package npm_run_build ## Build Code with Proto-Compiler
	...
	-cd src/ondewo-survey-api && git checkout -- '**/*.proto' && cd ../..
	cp src/package.json .
	cp src/README.md .
	cp src/RELEASE.md .
...
remove_npm_script: ## Removes the scripts section from the npm/ package.json copy (root package.json is left intact)
	...
	@if [ -n "$(start)" ] && [ -n "$(end)" ]; then perl -i -ne 'print unless $$. >= $(start) && $$. <= $(end)' npm/package.json; fi
