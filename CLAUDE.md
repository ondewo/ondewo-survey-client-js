...
This repo has submodules (`ondewo-proto-compiler`, `src/ondewo-survey-api`). If a submodule's working
...
```text
Release ONDEWO Survey Js Client ${ONDEWO_SURVEY_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO Survey Js Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO Survey Js Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`@ondewo/ondewo-survey-client-js`**, which is not always the repository name — the JS client
...
```bash
npm view @ondewo/ondewo-survey-client-js versions --json
```
