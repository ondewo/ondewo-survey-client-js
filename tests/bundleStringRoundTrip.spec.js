...
//
// This is an ARTEFACT test. `api/ondewo_survey_api.js` is a self-contained browser bundle: it embeds the
...
const BUNDLE_PATH = path.join(__dirname, '..', 'api', 'ondewo_survey_api.js');
 ...
 *
 * @returns {any} the `ondewo_survey_api` namespace object.
 */
function loadApiBundle() {
	...
	assert.ok(context.ondewo_survey_api, 'the bundle did not define the ondewo_survey_api global');
	return context.ondewo_survey_api;
}

runTestCase('a string field survives a binary round trip through the shipped bundle', () => {
	const api = loadApiBundle();
	assert.ok(api.AgentSurveyRequest, 'the bundle does not export AgentSurveyRequest');

	const original = new api.AgentSurveyRequest();
	...
	const decoded = api.AgentSurveyRequest.deserializeBinary(bytes);
	assert.equal(decoded.getSurveyId(), value, 'the string did not survive the round trip');
});

runTestCase('the bundle carries a protobuf runtime able to read the strings it writes', () => {
	const api = loadApiBundle();
	const message = new api.AgentSurveyRequest();
	message.setSurveyId('probe');
	...
	assert.doesNotThrow(
		() => api.AgentSurveyRequest.deserializeBinary(message.serializeBinary()),
